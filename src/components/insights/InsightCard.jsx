...
export function InsightCard({ insight }) {
  ...
  const confidenceText = combined.right
    ? formatLevel(combined.right)
    : formatLevel(insight?.confidence);
  ...
  return (
    <article
      className={`insight-card insight-${insight?.mode}`}
      aria-label={`${modeLabel} insight`}
    >
      <header className="insight-header">
        <span className={`insight-badge badge-${insight?.mode}`}>{modeLabel}</span>

        {confidenceText && (
          <span className="insight-confidence" aria-label="Confidence level">
            {" "}
            {confidenceText}
          </span>
        )}
      ...
      {insight?.rationale && (
        <p className="insight-rationale">
          <span className="insight-rationale-label">Why it matters:</span>{" "}
          {insight.rationale}
        </p>
      )}
