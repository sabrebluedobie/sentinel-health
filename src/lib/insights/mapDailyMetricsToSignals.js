...
export function mapDailyMetricsToSignals(rows) {
  ...
  const recent = sorted.slice(0, 7); // last week window

  // ---- Migraine frequency ----
  const migraineDays = recent.filter(r => Number.isFinite(r.migraine_count));
  if (migraineDays.length >= 2) {
    const totalMigraines = migraineDays.reduce(
      (sum, r) => sum + r.migraine_count,
      0
    );
  ...
  // ---- Glucose variability (only if data exists) ----
