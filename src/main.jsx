...
(function(){
  ...
  window.__bootlog = (...args)=>{ const t=new Date().toISOString().slice(11,19); box.textContent += `[${t}] `+args.map(a=>typeof a==="string"?a:JSON.stringify(a)).join(" ")+"\n"; };
})();
...
(function sanity(){
  ...
  if (miss.length){
    showFatal(`Missing env(s): ${miss.join(", ")}\n\nSet in Vercel → Project → Settings → Environment Variables (Preview + Production).`);
